  localparam NUM_CE = 1;  // Must be no more than 10 (6 ports taken by transport and IO connected CEs)
...
//  noc_block_duc #( .NUM_CHAINS(2), .NUM_HB(1), .CIC_MAX_INTERP(8), .NOC_ID(64'hD0C0_0000_0000_0002), .STR_SINK_FIFOSIZE(11), .MTU(10)) inst_noc_block_duc (
//    .bus_clk(bus_clk), .bus_rst(bus_rst),
//    .ce_clk(ce_clk), .ce_rst(ce_rst),
...
//    .debug(ce_debug[1]));
  ...
  generate
    for (n = 0; n < NUM_CE; n = n + 1) begin
      noc_block_axi_fifo_loopback #(.FIFOSIZE(11)) inst_noc_block_axi_fifo_loopback (
        .bus_clk(bus_clk), .bus_rst(bus_rst),
        .ce_clk(ce_clk), .ce_rst(ce_rst),
